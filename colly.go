...
func (c *Collector) checkRobots(u *url.URL) error {
	...
	if !ok {
		...
		reqCheck := func(_ *http.Request) bool { return true }
		respCheck := func(_ *http.Request, _ int, _ http.Header) bool { return true }

		resp, err := c.backend.Do(req, c.MaxBodySize, reqCheck, respCheck)
		if err != nil {
			return err
		}

		robot, err = robotstxt.FromBytes(resp.Body)
		if err != nil {
			return err
		}
		c.lock.Lock()
